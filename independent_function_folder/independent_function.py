...
def function_independence_interpolation_CMAGEP():
    ...
    predicted = data_prediction.predict_independence_data_CMAGEP(current_path, raw_data)
...
def function_independence_interpolation_SVM():
    ...
    raw_data = pandas.read_csv(os.path.join(current_path, r'raw_data.csv'), index_col=0)
    predicted = data_prediction.predict_independence_data_SVM(current_path, raw_data)
    # success_rate = numpy.corrcoef(predicted['0'], predicted['5'])

    return find_success_rate(predicted)


def function_independence_interpolation_KNN():
    default_folder = "data"
    current_path = pathlib.Path(__file__).parent.resolve().joinpath(default_folder)

    raw_data = pandas.read_csv(os.path.join(current_path, r'raw_data.csv'), index_col=0)
    predicted = data_prediction.predict_independence_data_KNN(current_path, raw_data)
    # success_rate = numpy.corrcoef(predicted['0'], predicted['5'])

    return find_success_rate(predicted)


def function_independence_interpolation_RandomForest():
    default_folder = "data"
    current_path = pathlib.Path(__file__).parent.resolve().joinpath(default_folder)

    raw_data = pandas.read_csv(os.path.join(current_path, r'raw_data.csv'), index_col=0)
    predicted = data_prediction.predict_independence_data_RandomForest(current_path, raw_data)
    # success_rate = numpy.corrcoef(predicted['0'], predicted['5'])
